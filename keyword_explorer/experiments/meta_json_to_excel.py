...
def read_json_files_to_dataframe(directory_path:str) -> Dict[str, pd.DataFrame]:
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.json'):
            with open(os.path.join(directory_path, filename), 'r', encoding='utf-8') as file: #인코딩 안되는거 같아서 cp949인코딩 추가해줬음
                ...
                experiments = json_data['experiment']  #아니 근데 읽어온 json파일에서 'experiment' 라는 key값이 없는데??
        ...
        if sheet_name != 'Summary':
            ...
            for index, row in df.iterrows():
                ...
                for s in source_list:
                    s = re.match(r'[^\w]*(.*)', s).group(1)  #문자와 숫자가 아닌 문자(특수 문자 또는 공백)로 시작하는 부분을 찾고, 이어지는 모든 문자를 캡처 
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        for sheet_name, df in dataframes.items():
            ...
            if sheet_name != 'Summary':
                f.write("\n=========================\nEngine: {}\n=========================\n".format(sheet_name))
                count = 0
                for row_dict in df.to_dict(orient='records'): #데이터 프레임을 '열':'값' 형태로 딕셔너리 변환 저장한다.
                    f.write("\n-------------------\n")
...
def main():
    ...
    path = tkinter.filedialog.askdirectory() #path지정
