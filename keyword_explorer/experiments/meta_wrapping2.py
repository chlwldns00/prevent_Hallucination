...
def repl_fun(match) -> str:
    index = random.randint(10000,99999)  #source 번호가 랜덤????
    index_list.append(index)
    return "(source {}).".format(index)

def add_markers(raw:str) -> str:
    cooked = re.sub(r'\. ', repl_fun, raw)  #정규식 이해가 필요할듯 -> 컨택스트에 source번호를 달때, 정규식을 사용하는듯보인다.
    return cooked
...
def main():
    ...
    with open("contexts.json", "r", encoding='utf-8') as f: ##디렉토리 내에 파일이 있는데 왜 여기서 오류가 나는거지
